...
#define VERSION_STRING "SV2019-05-14.0"
...
bool search_gaia_db(int hpx, struct db_entry &entry, std::string uuid, double longitude, double latitude, double radius, std::string where, OmniCoords &coords, boost::lockfree::stack<struct plot_data> &plot_stack, struct gaia_hist &hist)
{
...
void execute_gaia(uWS::HttpResponse *res, double ra, double dec, double radius, std::string where)
{
    int order = 4;
...
#pragma omp for schedule(dynamic)
        for (size_t i = 0; i < pixels->size(); i++)
        {
            ...
            try
            {
                ...
                abort_search = search_gaia_db(hpx, entry, uuid, longitude, latitude, rad, where, thread_coords[tid], plot_stack, global_hist);
            }
            catch (const std::out_of_range &err)
            {
                ...
                if (uri.find("GAIAWebQL.html") != std::string::npos)
                {
                    ...
                    if (pos != std::string::npos)
                    {
                        ...
                        if (std::isnan(ra) || std::isnan(dec) || std::isnan(radius))
                        {
                            ...
                            return execute_gaia(res, ra, dec, radius, where);
                    }
                    else
                    {
